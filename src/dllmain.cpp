...
std::vector<std::string> donators = {"Agha",
                                     "berrymuffin",
                                     "blackOS"
                                     "Boris Ilincic",
                                     "Damix",
...
BOOL WINAPI DllMain(HINSTANCE hDllHandle, DWORD nReason, LPVOID Reserved) {
  if (nReason == DLL_PROCESS_ATTACH) {
    ...
    Events::initScriptsEvent.after += []() {
      if (!gVerboseLogging) {
        gLogger->info("Enable 'VerboseLogging' in ModelExtras.ini to display "
    ...
    Events::initRwEvent.after += []() {
      ...
      if ((fxFuncs && !(ola || fla))) {
        ...
        MessageBox(RsGlobal.ps->window, str.c_str(), "LimitAdjuster required!",
                   MB_OK);
      }

      if (!cleo) {
        MessageBox(RsGlobal.ps->window,
                   "CLEO Library 4.4 or above is required!", "ModelExtras",
                   MB_OK);
    ...
    Events::initGameEvent += []() {
      ...
      */
      if (!std::filesystem::is_directory(PLUGIN_PATH((char *)MOD_NAME))) {
        std::string msg =
            std::format("{} folder not found. You need to put both '{}.asi' & "
      ...
      if (gConfig.ReadBoolean("CONFIG", "ShowIncompatibleWarning", true) &&
          (BackFireJDRInstalled || BackFireZAZInstalled || ImVehFtInstalled ||
           ImVehFtFixInstalled || AVSInstalled || EarShot)) {
        std::string str =
            "ModelExtras contain the functions of these plugins,\n\n";
        ...
        MessageBox(RsGlobal.ps->window, str.c_str(),
                   "Incompatible plugins found!", MB_OK);
        gLogger->error(str);
        exit(EXIT_FAILURE);
      }

      if (GrinchTrainer &&
          gConfig.ReadBoolean("CONFIG", "DeveloperMode", false)) {
        gLogger->info("GrinchTrainerSA found. Registering...");
        Events::processScriptsEvent += []() { TrainerInit(); };
      }
    };
