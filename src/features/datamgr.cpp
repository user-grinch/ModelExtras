...
void DataMgr::Reload(int model) {
  std::filesystem::directory_entry e = std::filesystem::directory_entry(
      std::filesystem::path(DataMgr::GetPath(model)));
  DataMgr::LoadFile(e);
}

const std::string &DataMgr::GetPath(int model) { return modelPath[model]; }
  ...
  try {
    ...
    try {
      ...
      if (jsonData.contains("metadata") &&
          jsonData["metadata"].contains("minver")) {
        if (gConfig.ReadBoolean("CONFIG", "ModelVersionCheck", true)) {
          ...
          if (ver > MOD_VERSION_NUMBER) {
            std::string text = std::format(
                "Model {} requires version [{}] but [{}] is installed.", model,
                ver, MOD_VERSION_NUMBER);
            MessageBox(RsGlobal.ps->window, text.c_str(),
                       "ModelExtras version mismatch!", MB_OK);
            gLogger->warn(text);
          }
        }
      ...
      gLogger->error("Failed to parse JSONC in file '{}': {}",
                     e.path().string(), ex.what());
    }
  } catch (const std::exception &ex) {
    std::u8string u8Path = e.path().u8string();
    std::string path(reinterpret_cast<const char *>(u8Path.data()),
                     u8Path.size());
...
void DataMgr::Parse() {
  ...
  for (const auto &e :
       std::filesystem::directory_iterator(MOD_DATA_PATH("data/"))) {
    LoadFile(e);
  }
  LOG_NO_LEVEL("");
  if (GetModuleHandle("modloader.asi")) {
    ...
    for (const auto &e : std::filesystem::recursive_directory_iterator(
             GAME_PATH((char *)"modloader/"))) {
      LoadFile(e);
    }
  }
}

nlohmann::json &DataMgr::Get(int model) { return data[model]; }
