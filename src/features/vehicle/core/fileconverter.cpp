...
bool Helper_OpenFile(const std::string &path, std::ifstream &infile,
...
bool Helper_CreateFile(const std::string &path, std::ofstream &outfile,
                       const std::string &logPrefix) {
  outfile.open(path);
  if (!outfile) {
...
void Helper_LoadPrepJson(const std::string &path, nlohmann::json &jsonData,
                         const std::string &logPrefix,
                         const std::string &clearKey) {
  if (std::filesystem::exists(path)) {
    ...
    if (jsonData.contains(clearKey)) {
      gLogger->warn("{}: {} already contains {}, replacing...", logPrefix, path,
                    clearKey);
      jsonData[clearKey] = {};
    }
  }
...
int Convert_EmlToJsonc(const std::string &emlPath) {
  ...
  std::string jsonPath =
      MOD_DATA_PATH("data\\") + std::to_string(model) + ".jsonc";
  ...
  while (std::getline(infile, line)) {
    ...
    state["type"] =
        type == 0 ? "directional"
                  : (type == 1 ? "inversed-directional" : "non-directional");
  }
...
const char *GetShadowTypeName(int index) {
  static const char *shadowTypeNames[] = {"round",
                                          "pointlight",
                                          "arealight",
                                          "bollard",
                                          "comet",
  ...
  while (std::getline(infile, line)) {
    ...
    if (line.starts_with("num_colors"))
      ...
      if (parsingColors) {
        int r, g, b;
        if (iss >> r >> g >> b)
          jsonData["carcols"]["colors"].push_back(
              {{"red", r}, {"green", g}, {"blue", b}});
      } else if (parsingVariations) {
        int a, b, c, d;
        if (iss >> a >> b >> c >> d)
          jsonData["carcols"]["variations"].push_back({{"primary", a},
                                                       {"secondary", b},
                                                       {"tertiary", c},
                                                       {"quaternary", d}});
      }
