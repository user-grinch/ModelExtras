...
void BackFireEffect::BackFireFX(CVehicle *pVeh, float x, float y, float z) {
  int handle = NULL;
  plugin::Command<Commands::CREATE_FX_SYSTEM_ON_CAR_WITH_DIRECTION>(
      "GUNFLASH", CPools::GetVehicleRef(pVeh), x, y, z, 0.0f, -25.0f, 0.0f, 1,
      &handle);
...
void BackFireEffect::BackFireSingle(CVehicle *pVeh) {
  ...
  CVehicleModelInfo *pInfo = static_cast<CVehicleModelInfo *>(
      CModelInfo::GetModelInfo(pVeh->m_nModelIndex));
  int handlingID = patch::Get<WORD>((int)pInfo + 74,
                                    false); //  CBaseModelInfo + 74 = handlingID
  tHandlingData *pHandlingData = reinterpret_cast<tHandlingData *>(
      0xC2B9DC + (handlingID * 224)); // sizeof(tHandlingData) = 224
  float vx = 0;
  ...
  vx = 0.0f;
  pos =
      pInfo->m_pVehicleStruct->m_avDummyPos[eVehicleDummies::EXHAUST_SECONDARY];
  if (!pos.IsZero()) {
    if (pHandlingData->m_bDoubleExhaust) {
      vx = pos.x * -1.0f;
    }
...
void BackFireEffect::Initialize(RwFrame *frame, CEntity *pVeh) {
  plugin::Events::initGameEvent += []() {
    std::string line =
        gConfig.ReadString("TABLE", "BackFireEffect_VehicleModels", "");
    onlySelected = gConfig.ReadBoolean(
        "VEHICLE_FEATURES", "BackfireEffect_OnlySelectedModels", true);
    Util::GetModelsFromIni(line, ValidModels);
  };

  plugin::Events::vehicleRenderEvent.before +=
      [](CVehicle *vehicle) { BackFireEffect::Process(vehicle); };
}

// Inspired by Junior's
// https://www.mixmods.com.br/2016/06/backfire-als-v2-5-mod-estalar-escapamento/
void BackFireEffect::Process(CVehicle *pVeh) {
  if (!pVeh->GetIsOnScreen() || pVeh->bEngineBroken || !pVeh->bEngineOn ||
      pVeh->bIsBig || pVeh->bIsVan || pVeh->bIsBus || pVeh->bIsRCVehicle) {
    return;
  }

  bool isValidVeh = std::find(ValidModels.begin(), ValidModels.end(),
                              pVeh->m_nModelIndex) != ValidModels.end();
  ...
  if (pVeh->m_nVehicleSubClass == VEHICLE_BIKE ||
    ...
    if (pVeh->m_pDriver && gchanging == 0 && rpm != 65535 && rpm > 100.0f &&
        speed > 5.0f) {
      BackFireSingle(pVeh);
    }
