...
void DirtFx::Initialize() {
  ...
  plugin::Events::vehicleSetModelEvent.after += [](CVehicle *pVeh, int model) {
    CVehicleModelInfo *pInfo =
        (CVehicleModelInfo *)CModelInfo::GetModelInfo(model);
    CTxdStore::PushCurrentTxd();
    CTxdStore::SetCurrentTxd(pInfo->m_nTxdIndex);
    RwTexDictionaryForAllTextures(
        RwTexDictionaryGetCurrent(),
        [](RwTexture *texture, void *pData) {
          RwTexture *pDirtTex = texture;
          std::string dirtName = pDirtTex->name;

          if (dirtName.starts_with("#") || dirtName.starts_with("remap") ||
              !dirtName.ends_with("_dt")) {
            return texture;
          }

          std::string cleanName =
              dirtName.substr(0, dirtName.find_last_of('_'));
          RwTexture *pCleanTex =
              TextureMgr::FindInDict(cleanName, RwTexDictionaryGetCurrent());
...
void DirtFx::ProcessTextures(CVehicle *pVeh, RpMaterial *pMat) {
  ...
  else if (texName == "vehicle_genericmud_truck" ||
           texName == "vehiclegrunge_iv")
...
void DirtFx::Shutdown() {
  ...
  for (int i = 0; i < 16; i++) {
    if (ms_aDirtTextures_2[i])
      RwTextureDestroy(ms_aDirtTextures_2[i]);
    if (ms_aDirtTextures_3[i])
      RwTextureDestroy(ms_aDirtTextures_3[i]);
    if (ms_aDirtTextures_4[i])
      RwTextureDestroy(
          ms_aDirtTextures_4[i]); // RwTextureDestroy(ms_aDirtTextures_5[i]);
  }
}
...
void DirtFx::InitialiseBlendTextureSingle(const char *CleanName,
                                          const char *DirtName,
...
void DirtFx::InitialiseDirtTextureSingle(const char *name,
                                         RwTexture **dirtTextureArray) {
...
void DirtFx::InitialiseDirtTextures() {
  ...
  InitialiseBlendTextureSingle("tyrewall_dirt", "tyrewall_dirt_dt",
                               ms_aDirtTextures_4);
  // InitialiseBlendTextureSingle((char *)"vehicle_generic_detail", (char
  // *)"vehicle_generic_detaild", ms_aDirtTextures_5);
}