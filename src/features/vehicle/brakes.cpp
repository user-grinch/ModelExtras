...
void FrontBrake::Initialize() {
  ModelInfoMgr::RegisterDummy([](CVehicle *pVeh, RwFrame *pFrame) {
    ...
    if (name.starts_with("x_fbrake") || name.starts_with("fc_fbrake")) {
      ...
      if (jsonData.contains("frontbrake") &&
          jsonData["frontbrake"].contains("maxrotation")) {
        data.m_nMaxRotation =
            jsonData["frontbrake"].value("maxrotation", 360.0f);
      }
      data.m_nWaitTime =
          static_cast<unsigned int>(abs(data.m_nMaxRotation / 5));
      data.pFrame = pFrame;
  ...
  ModelInfoMgr::RegisterRender([](CVehicle *pVeh) {
    ...
    uint timer = static_cast<int>(CTimer::m_snTimeInMilliseconds *
...
void RearBrake::Initialize() {
  ModelInfoMgr::RegisterDummy([](CVehicle *pVeh, RwFrame *pFrame) {
    ...
    if (name.starts_with("x_rbrake") || name.starts_with("fc_rbrake")) {
      VehData &data = vehData.Get(pVeh);
      auto &jsonData = DataMgr::Get(pVeh->m_nModelIndex);
      if (jsonData.contains("rearbrake") &&
          jsonData["rearbrake"].contains("maxrotation")) {
        data.m_nMaxRotation =
            jsonData["rearbrake"].value("maxrotation", data.m_nMaxRotation);
      }
      data.m_nWaitTime =
          static_cast<unsigned int>(abs(data.m_nMaxRotation / 5));
      data.pFrame = pFrame;
