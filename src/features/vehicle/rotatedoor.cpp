...
void RotateDoor::UpdateRotatingDoor(CVehicle *pVeh, DoorConfig &config,
                                    eDoors doorID) {
  if (!config.frame)
    return;
  ...
  float targetRot =
      ratio * (config.reverse ? 1.0f : -1.0f) * 90.0f - config.originalRot;

  float currentRot =
      MatrixUtil::GetRotationZ(&config.frame->modelling) - config.originalRot;
