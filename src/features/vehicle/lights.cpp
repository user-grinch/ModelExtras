...
CVector2D GetCarPathLinkPosition(CCarPathLinkAddress &address) {
  if (address.m_nAreaId != -1 && address.m_nCarPathLinkId != -1 &&
      ThePaths.m_pPathNodes[address.m_nAreaId]) {
    return CVector2D(
        static_cast<float>(
            ThePaths.m_pNaviNodes[address.m_nAreaId][address.m_nCarPathLinkId]
                .m_vecPosn.x) /
            8.0f,
        static_cast<float>(
            ThePaths.m_pNaviNodes[address.m_nAreaId][address.m_nCarPathLinkId]
...
void Lights::Initialize() {
  ...
  plugin::Events::initGameEvent += []() {
    gbGlobalIndicatorLights = gConfig.ReadBoolean(
        "VEHICLE_FEATURES", "StandardLights_GlobalIndicatorLights", false);

    gfGlobalCoronaSize = gConfig.ReadFloat("VISUAL", "LightCoronaSize", 0.3f);
    gGlobalShadowIntensity =
        gConfig.ReadInteger("VISUAL", "LightShadowIntensity", 220);
    gGlobalCoronaIntensity =
        gConfig.ReadInteger("VISUAL", "LightCoronaIntensity", 250);
  };

  Events::vehicleDtorEvent += [](CVehicle *pVeh) { m_Dummies.erase(pVeh); };
  ...
  ModelInfoMgr::RegisterDummy([](CVehicle *pVeh, RwFrame *frame) {
    ...
    VehicleDummyConfig c = {.pVeh = pVeh,
                            .frame = frame,
                            .corona = {.color = {255, 255, 255,
                                                 static_cast<unsigned char>(
                                                     gGlobalCoronaIntensity)}}};

    auto &dummies = m_Dummies[pVeh];

    if (name.starts_with("fogl") &&
        (STR_FOUND(name, "_l") || STR_FOUND(name, "_r"))) {
      c.dummyType = eDummyPos::Front;
      c.lightType = STR_FOUND(name, "_l") ? eLightType::FogLightLeft
                                          : eLightType::FogLightRight;
      c.corona.color = c.shadow.color = {
          255, 255, 255, static_cast<unsigned char>(gGlobalCoronaIntensity)};
      c.corona.lightingType = eLightingMode::Directional;
    } else if (name.starts_with("rev") &&
               (STR_FOUND(name, "_l") || STR_FOUND(name, "_r"))) {
      c.dummyType = eDummyPos::Rear;
      c.lightType = STR_FOUND(name, "_l") ? eLightType::ReverseLightLeft
                                          : eLightType::ReverseLightRight;
      c.corona.color = c.shadow.color = {
          255, 255, 255, static_cast<unsigned char>(gGlobalCoronaIntensity)};
      c.corona.lightingType = eLightingMode::Directional;
    } else if (name.starts_with("breakl") &&
               (STR_FOUND(name, "_l") || STR_FOUND(name, "_r"))) {
      c.dummyType = eDummyPos::Rear;
      c.lightType = STR_FOUND(name, "_l") ? eLightType::BrakeLightLeft
                                          : eLightType::BrakeLightRight;
      c.corona.color = c.shadow.color = {
          240, 0, 0, static_cast<unsigned char>(gGlobalCoronaIntensity)};
      c.corona.lightingType = eLightingMode::Directional;
      ...
      c.lightType =
          (d == "L") ? eLightType::STTLightLeft : eLightType::STTLightRight;
      c.dummyType = eDummyPos::Rear;
      c.corona.color = c.shadow.color = {
          240, 0, 0, static_cast<unsigned char>(gGlobalCoronaIntensity)};
      c.corona.lightingType = eLightingMode::Directional;
    } else if (auto d = Util::GetCharsAfterPrefix(name, "nabrakelight_", 1)) {
      c.lightType = (d == "L") ? eLightType::NABrakeLightLeft
                               : eLightType::NABrakeLightRight;
      c.dummyType = eDummyPos::Rear;
      c.corona.color = c.shadow.color = {
          240, 0, 0, static_cast<unsigned char>(gGlobalCoronaIntensity)};
      c.corona.lightingType = eLightingMode::Directional;
      ...
      c.corona.color = c.shadow.color = {
          250, 0, 0, static_cast<unsigned char>(gGlobalCoronaIntensity)};
      c.corona.lightingType = eLightingMode::Directional;
      ...
      c.corona.color = c.shadow.color = {
          250, 250, 250, static_cast<unsigned char>(gGlobalCoronaIntensity)};
      c.corona.lightingType = eLightingMode::Directional;
      ...
      if (d) {
        ...
        c.corona.color = c.shadow.color = {
            255, 128, 0, static_cast<unsigned char>(gGlobalCoronaIntensity)};
        c.corona.lightingType = eLightingMode::Directional;

        switch (d.value()[1]) {
        case 'F':
          c.lightType = isLeft ? eLightType::IndicatorLightLeftFront
                               : eLightType::IndicatorLightRightFront;
          c.dummyType = eDummyPos::Front;
          break;
        case 'R':
          c.lightType = isLeft ? eLightType::IndicatorLightLeftRear
                               : eLightType::IndicatorLightRightRear;
          c.dummyType = eDummyPos::Rear;
          break;
        case 'M':
          c.lightType = isLeft ? eLightType::IndicatorLightLeftMiddle
                               : eLightType::IndicatorLightRightMiddle;
  ...
  Events::processScriptsEvent += []() {
    ...
    if (pVeh && pVeh->IsDriver(FindPlayerPed()) &&
        pVeh->m_nOverrideLights != eLightOverride::ForceLightsOff &&
        !Util::IsEngineOff(pVeh)) {
      static size_t prev = 0;
      static uint32_t fogLightKey =
          gConfig.ReadInteger("KEYS", "FogLightKey", VK_J);
      if (KeyPressed(fogLightKey) &&
          IsMatAvail(pVeh,
                     {eLightType::FogLightLeft, eLightType::FogLightRight})) {
      ...
      static uint32_t longLightKey =
          gConfig.ReadInteger("KEYS", "LongLightKey", VK_G);
  ...
  Events::processScriptsEvent += []() {
    for (CVehicle *pVeh : CPools::ms_pVehiclePool) {
      ...
      if (DistanceBetweenPoints(pVeh->GetPosition(), TheCamera.GetPosition()) <
              150.0f ||
          pVeh->GetIsOnScreen()) {
        bool isLeftFrontOk =
            !Util::IsLightDamaged(pVeh, eLights::LIGHT_FRONT_LEFT);
        bool isRightFrontOk =
            !Util::IsLightDamaged(pVeh, eLights::LIGHT_FRONT_RIGHT);
        bool isHeadlightLeftOn =
            pVeh->m_renderLights.m_bLeftFront && isLeftFrontOk;
        bool isHeadlightRightOn =
            pVeh->m_renderLights.m_bRightFront && isRightFrontOk;
  ...
  ModelInfoMgr::RegisterRender([](CVehicle *pControlVeh) {
    ...
    if (pControlVeh->m_nOverrideLights == eLightOverride::ForceLightsOff ||
        pControlVeh->ms_forceVehicleLightsOff) {
      return;
    }
    ...
    if (pControlVeh->m_fHealth == 0 ||
        (Util::IsEngineOff(pControlVeh) &&
         pControlVeh->m_nOverrideLights != eLightOverride::ForceLightsOn)) {
      return;
    }

    bool isLeftFrontOk =
        !Util::IsLightDamaged(pControlVeh, eLights::LIGHT_FRONT_LEFT);
    bool isRightFrontOk =
        !Util::IsLightDamaged(pControlVeh, eLights::LIGHT_FRONT_RIGHT);
    bool isLeftRearOk =
        !(Util::IsLightDamaged(pTowedVeh, eLights::LIGHT_REAR_LEFT) ||
          Util::IsPanelDamaged(pTowedVeh, ePanels::WING_REAR_LEFT));
    bool isRightRearOk =
        !(Util::IsLightDamaged(pTowedVeh, eLights::LIGHT_REAR_RIGHT) ||
          Util::IsPanelDamaged(pTowedVeh, ePanels::WING_REAR_RIGHT));
    bool isHeadlightLeftOn =
        pControlVeh->m_renderLights.m_bLeftFront && isLeftFrontOk;
    bool isHeadlightRightOn =
        pControlVeh->m_renderLights.m_bRightFront && isRightFrontOk;
    ...
    if (data.m_bFogLightsOn) {
      RenderLights(pControlVeh, pTowedVeh, eLightType::FogLightLeft, true,
                   "foglight", 3.0f);
      RenderLights(pControlVeh, pTowedVeh, eLightType::FogLightRight, true,
                   "foglight", 3.0f);
    }
    ...
    if (pControlVeh->m_nVehicleSubClass == VEHICLE_AUTOMOBILE ||
        pControlVeh->m_nVehicleSubClass == VEHICLE_MTRUCK ||
        pControlVeh->m_nVehicleSubClass == VEHICLE_QUAD ||
        pControlVeh->m_nVehicleSubClass == VEHICLE_BIKE ||
        pControlVeh->m_nVehicleSubClass == VEHICLE_TRAILER) {
      bool isRevlightSupportedByModel =
          IsMatAvail(pTowedVeh, {eLightType::ReverseLightLeft,
                                 eLightType::ReverseLightRight});

      bool reverseLightsOn = !isBike && isRevlightSupportedByModel &&
                             pControlVeh->m_nCurrentGear == 0 &&
                             (Util::GetVehicleSpeed(pControlVeh) >= 0.001f) &&
                             pControlVeh->m_pDriver;
      if (reverseLightsOn) {
        if (isLeftRearOk) {
          RenderLights(pControlVeh, pTowedVeh, eLightType::ReverseLightLeft,
                       true, shdwName, shdwSz);
        }
        if (isRightRearOk) {
          RenderLights(pControlVeh, pTowedVeh, eLightType::ReverseLightRight,
                       true, shdwName, shdwSz);
        }
      }

      bool sttInstalled = IsMatAvail(
          pTowedVeh, {eLightType::STTLightLeft, eLightType::STTLightRight});
      // taillights/ brakelights
      if (pControlVeh->m_fBreakPedal && pControlVeh->m_pDriver) {
        if (sttInstalled) {
          if (isLeftRearOk) {
            RenderLights(pControlVeh, pTowedVeh, eLightType::STTLightLeft, true,
                         shdwName, shdwSz);
          }
          if (isRightRearOk) {
            RenderLights(pControlVeh, pTowedVeh, eLightType::STTLightRight,
                         true, shdwName, shdwSz);
          }
        } else {
          if (IsMatAvail(pTowedVeh, {eLightType::BrakeLightLeft,
                                     eLightType::BrakeLightRight})) {
            if (isLeftRearOk) {
              RenderLights(pControlVeh, pTowedVeh, eLightType::BrakeLightLeft,
                           true, shdwName, shdwSz);
            }
            if (isRightRearOk) {
              RenderLights(pControlVeh, pTowedVeh, eLightType::BrakeLightRight,
                           true, shdwName, shdwSz);
            }
          } else if (IsMatAvail(pTowedVeh, {eLightType::TailLightLeft,
                                            eLightType::TailLightRight})) {
            if (isLeftRearOk) {
              RenderLights(pControlVeh, pTowedVeh, eLightType::TailLightLeft,
                           true, shdwName, shdwSz);
            }
            if (isRightRearOk) {
              RenderLights(pControlVeh, pTowedVeh, eLightType::TailLightRight,
                           true, shdwName, shdwSz);
            }
          }
        }
        ...
        if (indState != eIndicatorState::BothOn) {
          if (indState != eIndicatorState::LeftOn && isLeftRearOk) {
            RenderLights(pControlVeh, pTowedVeh, eLightType::NABrakeLightLeft,
                         true, shdwName, shdwSz);
          }

          if (indState != eIndicatorState::RightOn && isRightRearOk) {
            RenderLights(pControlVeh, pTowedVeh, eLightType::NABrakeLightRight,
                         true, shdwName, shdwSz);
          }
        }
      }

      bool indicatorOn = data.m_bUsingGlobalIndicators &&
                         data.m_nIndicatorState != eIndicatorState::Off;
      bool tailLightFlag =
          Util::IsNightTime() ||
          pControlVeh->m_nOverrideLights == eLightOverride::ForceLightsOn ||
          pControlVeh->bLightsOn;
        ...
        if (sttInstalled) {
          ...
          auto tailLightsRender = [&](bool leftOk, bool rightOk) {
            if (IsMatAvail(pTowedVeh, {eLightType::TailLightLeft,
                                       eLightType::TailLightRight}) ||
                IsDummyAvail(pTowedVeh, {eLightType::TailLightLeft,
                                         eLightType::TailLightRight})) {
              if (leftOk) {
                RenderLights(pControlVeh, pTowedVeh, eLightType::TailLightLeft,
                             true, shdwName, shdwSz);
            ...
            } else if (IsMatAvail(pTowedVeh, {eLightType::BrakeLightLeft,
                                              eLightType::BrakeLightRight}) ||
                       IsDummyAvail(pTowedVeh, {eLightType::BrakeLightLeft,
                                                eLightType::BrakeLightRight})) {
              if (leftOk) {
                RenderLights(pControlVeh, pTowedVeh, eLightType::BrakeLightLeft,
                             true, shdwName, shdwSz);
              ...
              if (rightOk) {
                RenderLights(pControlVeh, pTowedVeh,
                             eLightType::BrakeLightRight, true, shdwName,
                             shdwSz);
              }
            }
          };
          ...
          if (indicatorOn) {
            if (data.m_nIndicatorState == eIndicatorState::BothOn) {
              tailLightsRender(isLeftRearOk && !indicatorsDelay,
                               isRightRearOk && !indicatorsDelay);
            }

            if (data.m_nIndicatorState == eIndicatorState::LeftOn) {
              tailLightsRender(isLeftRearOk && !indicatorsDelay,
                               isRightRearOk && tailLightFlag);
            }

            if (data.m_nIndicatorState == eIndicatorState::RightOn) {
              tailLightsRender(isLeftRearOk && tailLightFlag,
                               isRightRearOk && !indicatorsDelay);
            }
          } else {
    ...
    if (!gbGlobalIndicatorLights &&
        !IsMatAvail(pControlVeh, INDICATOR_LIGHTS_TYPE)) {
      return;
    }

    if (pControlVeh->m_pDriver == FindPlayerPed() &&
        (pControlVeh->m_nVehicleSubClass == VEHICLE_AUTOMOBILE ||
         pControlVeh->m_nVehicleSubClass == VEHICLE_BIKE ||
         pControlVeh->m_nVehicleSubClass == VEHICLE_QUAD ||
         pControlVeh->m_nVehicleSubClass == VEHICLE_MTRUCK)) {
      static uint32_t indicatorNoneKey =
          gConfig.ReadInteger("KEYS", "IndicatorLightNoneKey", VK_SHIFT);
      static uint32_t indicatorLeftKey =
          gConfig.ReadInteger("KEYS", "IndicatorLightLeftKey", VK_Z);
      static uint32_t indicatorRightKey =
          gConfig.ReadInteger("KEYS", "IndicatorLightRightKey", VK_C);
      static uint32_t indicatorBothKey =
          gConfig.ReadInteger("KEYS", "IndicatorLightBothKey", VK_X);
      ...
      data.m_nIndicatorState = eIndicatorState::Off;
      CVector2D prevPoint = GetCarPathLinkPosition(
          pControlVeh->m_autoPilot.m_nPreviousPathNodeInfo);
      CVector2D currPoint = GetCarPathLinkPosition(
          pControlVeh->m_autoPilot.m_nCurrentPathNodeInfo);
      CVector2D nextPoint =
          GetCarPathLinkPosition(pControlVeh->m_autoPilot.m_nNextPathNodeInfo);

      float angle = GetZAngleForPoint(nextPoint - currPoint) -
                    GetZAngleForPoint(currPoint - prevPoint);
      ...
      if (data.m_nIndicatorState == eIndicatorState::Off) {
        if (pControlVeh->m_autoPilot.m_nCurrentLane == 0 &&
            pControlVeh->m_autoPilot.m_nNextLane == 1) {
          data.m_nIndicatorState = eIndicatorState::RightOn;
        } else if (pControlVeh->m_autoPilot.m_nCurrentLane == 1 &&
                   pControlVeh->m_autoPilot.m_nNextLane == 0) {
          data.m_nIndicatorState = eIndicatorState::LeftOn;
        }
      }
    }
    ...
    if (gbGlobalIndicatorLights &&
        !IsMatAvail(pControlVeh, INDICATOR_LIGHTS_TYPE) &&
        !IsMatAvail(pControlVeh,
                    {eLightType::STTLightLeft, eLightType::STTLightRight})) {
      if ((pControlVeh->m_nVehicleSubClass == VEHICLE_AUTOMOBILE ||
           pControlVeh->m_nVehicleSubClass == VEHICLE_BIKE ||
           pControlVeh->m_nVehicleSubClass == VEHICLE_QUAD) &&
          (pControlVeh->GetVehicleAppearance() ==
               VEHICLE_APPEARANCE_AUTOMOBILE ||
           pControlVeh->GetVehicleAppearance() == VEHICLE_APPEARANCE_BIKE) &&
          pControlVeh->bEngineOn && pControlVeh->m_fHealth > 0 &&
          !pControlVeh->bIsDrowning && !pControlVeh->m_pAttachedTo) {
        data.m_bUsingGlobalIndicators = true;
      }
    } else {
      if (indState == eIndicatorState::BothOn ||
          indState == eIndicatorState::LeftOn) {
        if (isLeftFrontOk) {
          RenderLights(pControlVeh, pTowedVeh,
                       eLightType::IndicatorLightLeftFront, true, "indicator",
                       1.0f);
        }
        RenderLights(pControlVeh, pTowedVeh,
                     eLightType::IndicatorLightLeftMiddle, true, "indicator",
                     1.0f);
        if (isLeftRearOk) {
          RenderLights(pControlVeh, pTowedVeh,
                       eLightType::IndicatorLightLeftRear, true, "indicator",
                       1.0f);
          RenderLights(pControlVeh, pTowedVeh, eLightType::STTLightLeft, true,
                       shdwName, shdwSz, true);
        }
      }

      if (indState == eIndicatorState::BothOn ||
          indState == eIndicatorState::RightOn) {
        if (isRightFrontOk) {
          RenderLights(pControlVeh, pTowedVeh,
                       eLightType::IndicatorLightRightFront, true, "indicator",
                       1.0f);
        }
        RenderLights(pControlVeh, pTowedVeh,
                     eLightType::IndicatorLightRightMiddle, true, "indicator",
                     1.0f);
        if (isRightRearOk) {
          RenderLights(pControlVeh, pTowedVeh,
                       eLightType::IndicatorLightRightRear, true, "indicator",
                       1.0f);
          RenderLights(pControlVeh, pTowedVeh, eLightType::STTLightRight, true,
                       shdwName, shdwSz, true);
        }
      }
    }
    if (indState == eIndicatorState::BothOn ||
        indState == eIndicatorState::LeftOn) {
      if (isLeftRearOk) {
        RenderLights(pControlVeh, pTowedVeh, eLightType::NABrakeLightLeft, true,
                     "indicator", 1.0f);
      }
    }
...
void Lights::RenderLight(CVehicle *pVeh, eLightType state, bool shadows,
  ...
  if (IsDummyAvail(pVeh, state)) {
    for (auto e : m_Dummies[pVeh][state]) {
      ...
      if (c.dummyType == eDummyPos::Front &&
          Util::IsVehicleDoingWheelie(pVeh)) {
        continue;
      }

      if (shadows && c.shadow.render) {
        texture = (c.shadow.texture == "") ? texture : c.shadow.texture;
        RenderUtil::RegisterShadowDirectional(&e->Get(), texture,
                                              sz * c.shadow.size);
      }
    }
  }
...
void Lights::RenderLights(CVehicle *pControlVeh, CVehicle *pTowedVeh,
                          eLightType state, bool shadows, std::string texture,
                          float sz, bool highlight) {
  int model = pControlVeh->m_nModelIndex;
...
void Lights::RenderHeadlights(CVehicle *pControlVeh, bool isLeftOn,
                              bool isRightOn, bool realTime) {
  CVehicle *pTowedVeh = pControlVeh;
  VehLightData &data = m_VehData.Get(pControlVeh);
  ...
  if (pControlVeh->bLightsOn) {
    bool isFoggy = (CWeather::NewWeatherType == WEATHER_FOGGY_SF ||
                    CWeather::NewWeatherType == WEATHER_SANDSTORM_DESERT ||
                    CWeather::OldWeatherType == WEATHER_FOGGY_SF ||
                    CWeather::OldWeatherType == WEATHER_SANDSTORM_DESERT);
    std::string texName =
        data.m_bLongLightsOn ? "headlight_long" : "headlight_short";

    if (isLeftOn || isRightOn) {
      if (isLeftOn && GetLightState(pControlVeh, eLightType::HeadLightLeft)) {
        RenderLights(pControlVeh, pTowedVeh, eLightType::HeadLightLeft, true,
                     texName, headlightSz, isFoggy || data.m_bLongLightsOn);
      }
      if (isRightOn && GetLightState(pControlVeh, eLightType::HeadLightRight)) {
        RenderLights(pControlVeh, pTowedVeh, eLightType::HeadLightRight, true,
                     texName, headlightSz, isFoggy || data.m_bLongLightsOn);
      }
    }
  }
  ...
  if (gConfig.ReadBoolean("VEHICLE_FEATURES", "LightCoronas", false)) {
    ...
    if (c.corona.lightingType == eLightingMode::NonDirectional) {
      RenderUtil::RegisterCorona(
          pVeh, (reinterpret_cast<unsigned int>(pVeh) * 255) + 255 + id,
          c.position, c.corona.color, c.corona.size * szMul);
    } else {
      RenderUtil::RegisterCoronaDirectional(
          &dummy->Get(), c.rotation.angle, 180.0f, szMul, true,
          c.corona.lightingType == eLightingMode::Inversed);
    }
  }
}
...
bool Lights::IsDummyAvail(CVehicle *pVeh,
...
bool Lights::IsMatAvail(CVehicle *pVeh,
                        std::initializer_list<eLightType> states) {
...
bool Lights::IsIndicatorOn(CVehicle *pVeh) {
  return !Util::IsEngineOff(pVeh) &&
         (pVeh->m_nVehicleSubClass == VEHICLE_AUTOMOBILE ||
          pVeh->m_nVehicleSubClass == VEHICLE_BIKE) &&
         indicatorsDelay &&
         m_VehData.Get(pVeh).m_nIndicatorState != eIndicatorState::Off &&
         pVeh->m_nOverrideLights != eLightOverride::ForceLightsOff;
}
