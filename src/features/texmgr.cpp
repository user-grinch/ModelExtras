...
RwTexture *LoadPNGFromFile(const char *filename, RwUInt8 alpha) {
  ...
  RwImageFindRasterFormat(image, rwRASTERTYPETEXTURE | rwRASTERFORMAT888,
...
RwTexture *TextureMgr::RwReadTexture(const char *name, char *Maskname) {
  return ((RwTexture * (__cdecl *)(char const *, char const *))0x4C7510)(
      name, Maskname);
}

RwTexture *TextureMgr::Get(std::string name, RwUInt8 alpha) {
  if (Textures.contains(name) && Textures[name].contains(alpha) &&
      Textures[name][alpha]) {
    return Textures[name][alpha];
  }

  static auto pDict =
      CFileLoader::LoadTexDictionary(MOD_DATA_PATH("ME_TEXDB.TXD"));
...
RwTexture *TextureMgr::FindOnTextureInDict(RpMaterial *pMat,
...
void TextureMgr::SetAlpha(RwTexture *texture, RwUInt8 alpha) {
  ...
  RwRaster *newRaster = RwRasterCreate(
      width, height, 32, rwRASTERTYPETEXTURE | rwRASTERFORMAT8888);
...
RwTexture *TextureMgr::FindInDict(std::string name, RwTexDictionary *pDict,
  ...
  if (fallback) {
    if (!pTex) {
      LOG_VERBOSE("TextureMgr: Unable to find '{}' in the vehicle's TXD file. "
                  "Searching in the ModelExtras TXD file instead.",
                  name);
      pTex = TextureMgr::Get(name);
    }

    if (!pTex) {
      LOG_VERBOSE("TextureMgr: Unable to find '{}' in the ModelExtras TXD "
                  "file. Searching in the vehicle TXD file instead.",
                  name);
      pTex = RwTexDictionaryFindNamedTexture(CVehicleModelInfo::ms_pVehicleTxd,
                                             name.c_str());
    }

    if (!pTex) {
      LOG_VERBOSE("TextureMgr: Unable to find '{}' in the vehicle TXD file. "
