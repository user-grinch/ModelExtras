...
void ModelInfoMgr::Initialize() {
  ...
  MEEvents::vehRenderEvent.before +=
      [](CVehicle *pVeh) { ModelInfoMgr::OnRender(pVeh); };
...
void ModelInfoMgr::RegisterMaterialColProvider(
    MaterialColProviderCallback_t mat) {
...
RpMaterial *ModelInfoMgr::SetEditableMaterialsCB(RpMaterial *material,
  ...
  if (iLightIndex != eLightType::UnknownLight) {
    ...
    (*ppEntries)->m_pValue =
        *reinterpret_cast<void **>(RpMaterialGetColor(material));
    (*ppEntries)++;
