  ...
  plugin::Events::processScriptsEvent += [] {
    ...
    if (cur - prev > 5000) {
      for (auto it = m_NeedToFree.begin(); it != m_NeedToFree.end();) {
        ...
        if (state == eAudioStreamState::Stopped ||
            state == eAudioStreamState::Paused) {
...
void AudioMgr::PlayFileSound(const std::string &path, CEntity *pEntity,
                             float volume, bool cached) {
  StreamHandle handle = NULL;
  ...
  if (state != eAudioStreamState::Playing) {
    ...
    if (pEntity->m_nType == ENTITY_TYPE_VEHICLE) {
      ...
      plugin::Command<SET_PLAY_3D_AUDIO_STREAM_AT_COORDS>(handle, pos.x, pos.y,
                                                          pos.z);
    }
    plugin::Command<SET_AUDIO_STREAM_STATE>(
        handle, static_cast<int>(eAudioStreamState::Playing));
  }
}
