...
void PedRemap::Initialize() {
	...
	Events::initRwEvent += []
	{
		...
		ORIGINAL_AssignRemapTxd = injector::ReadMemory<uintptr_t>(0x5B62C2 + 1, true);
				...
				RpClumpForAllAtomics(ped->m_pRwClump, [](RpAtomic *atomic, void *data)
				{
					...
					if (atomic->geometry)
					{
						RpGeometryForAllMaterials(atomic->geometry, [](RpMaterial *material, void *data)
						{
							...
							{
								if (info->curRemapNum[i] >= 0 && (int)info->originalRemap[i] > 0)
								{
									if (info->originalRemap[i] == material->texture)
									{
										if (!info->remaps[i].empty())
										{
											std::list<RwTexture*>::iterator it = info->remaps[i].begin();
