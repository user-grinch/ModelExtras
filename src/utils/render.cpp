...
inline CVector2D GetPerpRight(const CVector2D &vec) { return {vec.y, -vec.x}; }

inline CVector2D Rotate2D(const CVector2D &vec, float angle) {
  float cosA = cos(angle);
  float sinA = sin(angle);
  return CVector2D(vec.x * cosA - vec.y * sinA, vec.x * sinA + vec.y * cosA);
}
...
void RenderUtil::RegisterCorona(CEntity *pEntity, int coronaID, CVector pos,
  ...
  CCoronas::RegisterCorona(coronaID, pEntity, col.r, col.g, col.b, col.a, pos,
                           size * CORONA_SZ_MUL, 260.0f, CORONATYPE_SHINYSTAR,
                           FLARETYPE_NONE, true, false, 0, 0.0f, false, 0.3f, 0,
                           30.0f, false, false);
};

void RenderUtil::RegisterCoronaDirectional(const VehicleDummyConfig *pConfig,
                                           float angle, float radius,
                                           float szMul, bool checks,
                                           bool inversed) {
  ...
  if (!IsDummyPointingUp(mat)) {
    if (checks && IsShadowTowardVehicle((CMatrix *)&pConfig->frame->ltm,
                                        pConfig->pVeh->GetPosition())) {
      angle += 180.0f;
    }
    ...
    float vehicleAngle =
        Util::NormalizeAngle(Util::RadToDeg(pConfig->pVeh->GetHeading()));
    float cameraAngle =
        Util::NormalizeAngle(Util::RadToDeg(TheCamera.GetHeading()));
    ...
    if (diffAngle < cutoff + FADE_RANGE) {
      ...
      float adjustedAngle =
          FADE_RANGE - (diffAngle - (360.0f - cutoff - FADE_RANGE));
      float mul = std::fabs(adjustedAngle / FADE_RANGE);
      col.a *= mul;
    }
    ...
    CPointLights::AddLight(PLTYPE_SPOTLIGHT, mat.pos, mat.up, 10.0f,
                           col.r / 255.0, col.g / 255.0, col.b / 255.0, 0, 0,
                           0);
  }
  RegisterCorona(pConfig->pVeh, reinterpret_cast<int32_t>(pConfig),
                 pConfig->position, col, sz);
}

extern int gGlobalShadowIntensity;

void RenderUtil::RegisterShadowDirectional(const VehicleDummyConfig *pConfig,
                                           const std::string &shadwTexName,
                                           float shdwSz) {
  const float SHDW_SZ_MUL = 2.0f;
  if (!pConfig->pVeh || !pConfig || shdwSz == 0.0f ||
      !gConfig.ReadBoolean("VEHICLE_FEATURES", "LightShadows", false)) {
    return;
  }

  float lightHeightLimit =
      gConfig.ReadBoolean("TWEAKS", "LightHeightLimit", false);
  if (lightHeightLimit != 0.0f &&
      pConfig->frame->modelling.pos.z >= lightHeightLimit) {
    return;
  }
  ...
  CMatrix mat = *(CMatrix *)&pConfig->frame->ltm;
  if (pConfig->shadow.rotationChecks &&
      IsShadowTowardVehicle((CMatrix *)&pConfig->frame->ltm,
                            pConfig->pVeh->GetPosition())) {
    RotateMatrix180Z(mat);
  }
  ...
  CVector worldOffset =
      mat.pos - vehMat.pos; // world-space vector from vehicle to dummy
  ...
  rotatedOffset += CVector(rotatedLightDir.x, rotatedLightDir.y, 0.0f) *
                   (shdwSz * SHDW_SZ_MUL + 0.2f);

  CVector2D shdwFront(rotatedLightDir.x * (shdwSz * SHDW_SZ_MUL),
                      rotatedLightDir.y * (shdwSz * SHDW_SZ_MUL));
  ...
  CVector shdwPos = pConfig->pVeh->GetPosition() +
                    CVector(rotatedOffset.x, rotatedOffset.y, 2.0f);
  CShadows::StoreCarLightShadow(
      pConfig->pVeh, reinterpret_cast<int32_t>(pConfig), pTex, &shdwPos,
      shdwFront.x, shdwFront.y, shdwSide.x, shdwSide.y, pConfig->shadow.color.r,
      pConfig->shadow.color.g, pConfig->shadow.color.b, 7.0f);
}

void RenderUtil::RegisterShadow(CEntity *pEntity, CVector position, CRGBA col,
                                float angle, eDummyPos dummyPos,
                                const std::string &shadwTexName,
                                CVector2D shdwSz, CVector2D shdwOffset,
                                RwTexture *pTexture) {
  ...
  auto RotateVector2D = [angleRad](const CVector &v) -> CVector {
    return {v.x * cos(angleRad) - v.y * sin(angleRad),
            v.x * sin(angleRad) + v.y * cos(angleRad), v.z};
  };
  ...
  CVector nOffset = {
      ...
      shdwOffset.x * sin(angleRad) + shdwOffset.y * cos(angleRad), 0.0f};

  CVector shdwPos =
      pEntity->TransformFromObjectSpace(position + nOffset + nSize);
  shdwPos.z = CWorld::FindGroundZFor3DCoord(
                  shdwPos.x, shdwPos.y, shdwPos.z + 100.0f, NULL, &pEntity) +
              2.0f;
  ...
  if (pTex) {
    CShadows::StoreShadowToBeRendered(2, pTex, &shdwPos, up.x, up.y, right.x,
                                      right.y, col.a, col.r, col.g, col.b, 6.0f,
                                      false, 1.0f, 0, true);
  }
}