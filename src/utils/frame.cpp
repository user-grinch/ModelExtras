...
bool FrameUtil::IsOkAtomicVisible(RwFrame *frame) {
  if (frame && !rwLinkListEmpty(&frame->objectList)) {
    ...
    do {
      atomic = rwLLLinkGetData(current, RwObjectHasFrame, lFrame);
      bool isOkAtomic = (CVisibilityPlugins::GetAtomicId((RpAtomic *)atomic) &
                         3) == 1; // 1 = Ok, 2 = Damaged, 3 = None
...
// VehFuncs
RwFrame *FrameUtil::Clone(RwFrame *frame, RpClump *clump, RwFrame *parent,
                          bool isRoot) {
  RwFrame *newFrame;
  if (isRoot) {
    *(uint32_t *)0xC1CB58 = (uint32_t)clump;
    ...
    if (RwFrame *nextFrame = frame->child)
      Clone(nextFrame, clump, parent, false);
  } else {
    ...
    if (atomic) {
      ...
      // CVisibilityPlugins::SetAtomicRenderCallback(newAtomic, (RpAtomic
      // *(*)(RpAtomic *))0x007323C0);
    }
    ...
    if (RwFrame *nextFrame = frame->child)
      Clone(nextFrame, clump, newFrame, false);
    // if (RwFrame * nextFrame = frame->next)  CloneNode(nextFrame, clump,
    // parent, false);
  }
  return newFrame;
}