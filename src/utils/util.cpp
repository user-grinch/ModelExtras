...
double Util::RadToDeg(double rad) { return rad * (180.0 / PI); }

double Util::DegToRad(double deg) { return deg * (PI / 180.0); }
...
float Util::GetVehicleSpeedRealistic(CVehicle *vehicle) {
  ...
  CVehicleModelInfo *vehicleModelInfo =
      (CVehicleModelInfo *)CModelInfo::GetModelInfo(vehicle->m_nModelIndex);
  if (vehicle->m_nVehicleSubClass == VEHICLE_BIKE ||
      vehicle->m_nVehicleSubClass == VEHICLE_BMX) {
    CBike *bike = (CBike *)vehicle;
    wheelSpeed =
        ((bike->m_fWheelSpeed[0] * vehicleModelInfo->m_fWheelSizeFront) +
    ...
    wheelSpeed =
        ((automobile->m_fWheelSpeed[0] +
          automobile->m_fWheelSpeed[1] * vehicleModelInfo->m_fWheelSizeFront) +
         (automobile->m_fWheelSpeed[2] +
          automobile->m_fWheelSpeed[3] * vehicleModelInfo->m_fWheelSizeRear)) /
        4.0f;
  } else {
...
unsigned int Util::GetEntityModel(void *ptr, eModelEntityType type) {
  ...
  if (type == eModelEntityType::Weapon) {
    CWeaponInfo *pWeaponInfo = CWeaponInfo::GetWeaponInfo(
        reinterpret_cast<CWeapon *>(ptr)->m_eWeaponType,
        FindPlayerPed()->GetWeaponSkill(
            reinterpret_cast<CWeapon *>(ptr)->m_eWeaponType));
    if (pWeaponInfo) {
...
std::optional<int> Util::GetDigitsAfter(const std::string &str,
                                        const std::string &prefix) {
  if (str.rfind(prefix, 0) == 0) {
    std::string numberPart = str.substr(prefix.size());
    if (!numberPart.empty() &&
        std::all_of(numberPart.begin(), numberPart.end(),
...
std::optional<std::string> Util::GetCharsAfterPrefix(const std::string &str,
                                                     const std::string &prefix,
                                                     size_t num_chars) {
...
bool Util::IsNightTime() { return CClock::GetIsTimeInRange(20, 6); }
...
CVector Util::UpdateRelativeToBoundingBox(CVehicle *pVeh, eDummyPos dummyPos,
                                          CVector shdwPos, CVector up,
                                          CVector right) {
  CVehicleModelInfo *pInfo =
      (CVehicleModelInfo *)CModelInfo::GetModelInfo(pVeh->m_nModelIndex);
  CVector min =
      pInfo->m_pColModel->m_boundBox.m_vecMin + CVector(mx1, my1, mz1);
  CVector max =
      pInfo->m_pColModel->m_boundBox.m_vecMax + CVector(mx2, my2, mz2);
...
bool Util::IsVehicleDoingWheelie(CVehicle *pVeh) {
  return pVeh->m_nVehicleSubClass == VEHICLE_BIKE &&
         (Util::GetVehiclePitch(pVeh) > 30.0f ||
          pVeh->GetNumContactWheels() < 2);
}